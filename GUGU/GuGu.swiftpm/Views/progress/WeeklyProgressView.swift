...
struct WeeklyProgressView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        let sampleData = WeeklyAnalytics(
            id: UUID(),
            ...
            dailyData: (0..<7).map { day in
                DailyProgressData(
                    id: UUID(),
                    date: Calendar.current.date(byAdding: .day, value: day, to: Calendar.current.startOfWeek()) ?? Date(),
                    goalId: UUID(),
                    completedCount: Int.random(in: 0...8),
                    targetCount: 8,
                    completionTime: [Date()]
                )
            }
        ...
        Group {
            NavigationView {
                WeeklyProgressView(analytics: sampleData)
