...
class MealGoalTracker: GoalTracker {
    ...
    @Published private(set) var currentCooldown: Date?
    ...
    func canLogMeal(_ type: MealType) -> Bool {
        ...
        // If there's a current cooldown, check if it's expired
        if let cooldownEnd = currentCooldown {
    ...
    func logMeal(_ type: MealType) {
        ...
        withAnimation(.spring(response: 0.3)) {
            ...
            currentCooldown = nextAvailableTime
            lastMealLogged = type
    ...
    func isCoolingDown(_ type: MealType) -> Bool {
        guard let cooldownEnd = currentCooldown else { return false }
