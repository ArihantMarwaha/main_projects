...
class PetActivityManager: ObservableObject {
    @Published private(set) var petData: PetData
    private let repository: PetRepository
    let rewardSystem: RewardSystem
    @Published private(set) var progressionSystem: ProgressionSystem
    ...
    init(repository: PetRepository = PetRepository(), rewardSystem: RewardSystem = RewardSystem()) {
        self.repository = repository
        self.rewardSystem = rewardSystem
        self.progressionSystem = ProgressionSystem()
        self.achievementTracker = AchievementTracker(rewardSystem: rewardSystem)
        self.petData = repository.loadPetData() ?? PetData(
            name: "",
            birthday: Date(),
            dailyProgress: 0,
    ...
    nonisolated func updateProgress(for goalType: String) {
        Task { @MainActor in
            await self.updateProgressOnMain(for: goalType)
        }
    }
    ...
    func handleCooldownExpired(for goalId: UUID) {
    ...
    internal func updatePetData(_ update: (inout PetData) -> Void) {
        withAnimation {
            update(&petData)
            savePetData()
        }
